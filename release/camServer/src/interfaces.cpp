...
void iface_server::init_socket() {
    try {
        ...
        std::cerr << "[iface_server] Could not initialize because of ZeroMQ exception" << std::endl;
        std::cerr << "[iface_server]" << e.what() << std::endl;
...
int iface_server::get_status() {
    if(status == CAMSERVER_TCP_ERROR) {
        ...
        try {
            ...
            std::cerr << "[iface_server] Warning! Exception while getting status: " << e.what() << std::endl;
        }
...
void iface_node::init_socket() {
    try {
        ...
        std::cerr << "[iface_node] Could not initialize because of ZeroMQ exception" << std::endl;
        std::cerr << "[iface_node]" << e.what() << std::endl;
...
int iface_node::get_status() {
    ...
    if(status == CAMNODE_TCP_ERROR) {
        ...
        try {
            ...
            std::cerr << "[iface_node] Warning, exception while getting status: " << e.what() << std::endl;
        }
    }
